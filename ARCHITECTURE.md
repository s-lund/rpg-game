...
**Extending the effect union (M9+):** the frozen M1 pipeline test compiles an exhaustive switch over `Effect["kind"]`, so the exported `Effect` type is frozen at its M1 kinds. Post-freeze kinds (e.g. M9's `SpendSpellSlot`) are added to **`AnyEffect`** in `src/core/effects/types.ts` — the type every pipeline function (`apply`, `applyAll`, `resolveAction`, replay) actually accepts — and each new kind ships its own contract test covering the same one-effect-one-event and replay guarantees. Add the matching event case to `effectFromEvent` in `engine.ts` or replay silently drops it.
